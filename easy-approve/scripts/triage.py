...
def classify(pr: dict, buckets: set[str]) -> str:
    """zero-risk / validatable / needs-real-review — never 'approve'.
...
def main() -> int:
        ...
        rows.append(
            {
                ...
                "bucket": classify(pr, buckets),
            }
        )

    # Drift costs an API call each, so only ask for the PRs that will be acted on.
    candidates = [r for r in rows if r["bucket"] != "needs-real-review"]
    ...
    order = {"validatable": 0, "zero-risk": 1, "needs-real-review": 2}
    ...
    print(f"| # | bucket | paths | lines | CI | drift | author | title |")
    print(f"|---|---|---|---|---|---|---|---|")
    for r in rows:
        flags = []
        if r["stacked"]:
        ...
        print(
            ...
            f"{r['lines']} | {r['ci']} | {r.get('drift', '-')} | {r['author']} | {title} |"
        )
    return 0
